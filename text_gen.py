...
# 파일 경로 설정
file_path = 'FallRisk_En_No1.tsv'  # 업로드된 파일 경로
...
for idx, row in df_text_lines.iterrows():
    # 파일명 설정 (숫자를 4자리로 패딩)
    file_name = f'FallRisk_En_No_{str(idx+1).zfill(4)}.txt'
...
[f'FallRisk_En_No_{str(idx+1).zfill(4)}.txt' for idx in range(len(df_text_lines))]
